...
void main()
{
	...
	while (1) 
	{
	printf_s("Insert sentence ('~' to exit): ");
	gets_s(input);
	if (input[0] == '~')
		break;
	printf_s("%.2f\n",calculator(input));
...
int loader(char *input, conveer *line)//Разбивает входную строку на элементы и записывает в массив
{
	...
	while(input[i]!='\0')
	{
		...
		if(is_operator(input[i]))//операторы  сохраняем в вых массив
		{
			...
			{
				printf_s("One operator cant follow second");
				if (getchar())
					return 0;
			}
			line[j].type = input[i];
		...
		if (regex_search(input,sym))//если введены недопустимые символы-вывести ошибку
		{
				printf_s("restricted symbols");
				if (getchar())
					return 0;
		}
	...
	if (b != 0)//Пропущена скобка
	{
		if (b < 0)
		{
			printf_s("missed '('");//открывающая
			if (getchar())
				return 0;
		}
		if (b > 0)
		{
			printf_s("missed ')'");//закрывающая
			if (getchar())
				return 0;
		}
...
double solve(conveer *line)//Вычисляет обратную польскую запись
{
	...
	while (line[inposs].type != ' ')
	{
		...
		if (line[inposs].type == '*')//обрабатываем произведение
		{
			if (stposs - 2 == -1)
			{
				printf_s(" '*' is 2-operand operation \n");
				if (getchar())
					return 0;
			}
			else
